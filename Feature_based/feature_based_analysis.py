...
Outputs:
- Outputs/<dataset_name>_final.nk
- Outputs/<dataset_name>_final_ascii.txt
"""
...
# Folder containing this script: Feature_based/
BASE_DIR = Path(__file__).resolve().parent

# CSV feature files: Feature_based/Dataset/
DATASET_DIR = BASE_DIR / "Dataset"

# Output files: Feature_based/Outputs/
OUTPUT_DIR = BASE_DIR / "Outputs"

# FASTA files: LLC/ (sibling of Feature_based)
LLC_DIR = BASE_DIR.parent / "LLC"
...
def _debug_matrix(D: np.ndarray, name: str):
    ...
    print(f"  n={n}")
    print(
        f"  offdiag min/median/max = "
        f"{float(off.min())} / "
        f"{float(np.median(off))} / "
        f"{float(off.max())}"
    )
    print(
        f"  zero offdiag edges = "
        f"{int(np.sum(off == 0.0))} out of {off.size}"
    )
    print(
        f"  unique distances (rounded 1e-12) = "
        f"{np.unique(np.round(off, 12)).size}"
    )
...
def _scale_for_primMST(D: np.ndarray, labels, scale=DIST_SCALE):
    ...
    zeros_if_int = int(
        np.sum(np.floor(off).astype(np.int64) == 0)
    )

    print(
        f"[DEBUG] after scaling x{scale}: "
        f"zeros_if_int_cast = {zeros_if_int} out of {off.size}"
    )
...
class FeatureBasedClonalTree:
    def __init__(self):
        ...
        print(f"📁 Output path: {self.results_path}")
        print(f"📁 LLC path: {LLC_DIR}")
        print()
    ...
    def load_features_for_dataset(self, dataset_name: str):
        ...
        print(
            f"✅ Loaded {len(full_map)} feature rows "
            f"with {len(feature_names)} features"
        )
        print(
            f"   (base-id map size: {len(base_map)})"
        )
    ...
    def _get_feature_value(
        ...
        if lab in self.features_data_full:
            return self.features_data_full[lab].get(
                feature_name,
                np.nan
            )

        if base in self.features_data_base:
            return self.features_data_base[base].get(
                feature_name,
                np.nan
            )
    ...
    def create_single_feature_adjacency(
        self,
        labels,
        feature_name,
        normalize="rank"
    ...
    def create_multi_feature_adjacency(
        ...
        # per-label tiny jitter in feature space
        for i, lab in enumerate(labels):
            for j, f in enumerate(feature_list):
        ...
        print(
            f"[DEBUG] multi-feature adjacency raw: "
            f"n={n}, k={k}, "
            f"missing_raw_values={missing_raw}"
        )
    ...
    def run_feature_based_clonaltree(
        ...
        if self.load_features_for_dataset(
            dataset_name
        ) is None:
            return None

        labels, root, arraySeqs, abundance, dico = readFastaAbundance(
            fasta_file
        )
        ...
        print("\n" + "=" * 80)
        print(
            "DEBUG: readFastaAbundance() outputs"
        )
        print("=" * 80)
        ...
        for lab in labels:
            ...
            if (
                (str(lab) in self.features_data_full)
                or
                (base in self.features_data_base)
                or
                (
                    lab == "naive@1"
                    and
                    "naive" in self.features_data_base
                )
            ):
                present += 1

            else:
                missing_in_features.append(
                    lab
                )
        ...
        print(
            f"[features] present for "
            f"{present}/{len(labels)} labels"
        )

        print(
            f"[features] missing for "
            f"{len(missing_in_features)} labels "
            f"(kept, will be imputed)"
        )
        ...
        ranked = self.rank_features_global(
            [
                str(l).split("@")[0]
        ...
        _debug_matrix(
            adjMatrix,
            f"scaled adjacency passed to primMST "
            f"(x{DIST_SCALE})"
        )
        ...
        print(
            f"[tree] leaves = "
            f"{len(leaf_names)} ; "
            f"expected = {len(used_labels)}"
        )

        missing_from_tree = sorted(
            set(map(str, used_labels))
            -
            set(map(str, leaf_names))
        )

        if missing_from_tree:

            print(
                "[tree][WARN] leaf mismatch detected!"
            )

            print(
                "  missing_from_tree (first 30):",
                missing_from_tree[:30]
            )

        # Only rename leaves if leaf base exists in abundance
        for leaf in tree.iter_leaves():
            ...
            if base in used_abundance:
                ...
                leaf.name = (
                    f"{base}@"
                    f"{used_abundance[base]}"
                )

        # Outputs go directly to Feature_based/Outputs/
        ...
        out_newick = (
            out_dir
            /
            f"{dataset_name}_final.nk"
        )

        tree.write(
            outfile=str(out_newick),
            format=1
        )
        ...
        out_ascii = (
            out_dir
            /
            f"{dataset_name}_final_ascii.txt"
        )
...
def main():
    ...
    # FASTA files are in project_root/LLC/
    fasta_dir = LLC_DIR
    ...
    for num in range(
        ...
        try:
            ...
            if res:
                ...
                print(
                    f"❌ {dataset_name}: "
                    f"run_feature_based_clonaltree "
                    f"returned None"
                )
    ...
    print(
        f"⏭️ skipped (missing fasta): "
        f"{skipped}"
    )

    print(
        f"❌ failed (errors/None): "
        f"{failed}"
    )
