...
def updateDistances(parent, node, sisters, cost, labels):
	...
	# Calculate appropriate costNone based on actual distance range
	# Instead of hardcoding to 1, use a small fraction of the minimum distance
	min_distance = float('inf')
	for s in sisters:
		idS = labels.index(s)
		min_distance = min(min_distance, cost[idS][idPar], cost[idNode][idPar])
	
	# Use 10% of the minimum distance, but at least 0.01 to avoid zero
	costNone = max(min_distance * 0.1, 0.01)
	...
	node_distance = max(cost[idNode][idPar] - costNone, 0.001)  # At least 0.001
	distances.append(node_distance)
	
	for s in sisters:
		idS = labels.index(s)
		sister_distance = max(cost[idS][idPar] - costNone, 0.001)  # At least 0.001
		distances.append(sister_distance)
	return  distances
